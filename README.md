...
- **AI 调用稳健化**：429 立即切兜底模型；解析连续失败立即切兜底；服务不可用走超时（默认 5 分钟，可通过 `AI_CALL_TIMEOUT_SECONDS` 调整）+ 熔断（默认 3 次失败熔断 5 分钟）；厂商特定思考参数自动注入
...
| `AI_ANALYSIS_CONCURRENCY` | 单任务内 AI 分析并发数，默认 `1`（串行）；设为大于 1 可同时分析多个商品 | ❌ |
| `AI_CALL_TIMEOUT_SECONDS` | 单次 AI 请求超时秒数，默认 `300`（5 分钟） | ❌ |
| `PROXY_URL` | AI 请求专用代理 | ❌ |
